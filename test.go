package main

import (
	"fmt"
)

var SBoxI1 = [256]byte{
	,
}

// InverseSBoxI1 will be computed automatically
var InverseSBoxI1 [256]byte

func init() {
	for i, val := range SBoxI1 {
		InverseSBoxI1[val] = byte(i)
	}
}

func main() {
	fmt.Println("S-Box I1:")
	fmt.Println(SBoxI1)
	fmt.Println("\nInverse S-Box I1:")
	fmt.Println(InverseSBoxI1)
}
